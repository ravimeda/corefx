        ...
        "CMake"
        {
            return "CMake is a prerequisite to build this repository but it was not found on the path. " + "`r`n" +
                    "Please try one of the following options to acquire CMake version $declaredVersion. " + "`r`n" +
                        ...
                        "2. Run the script located at $([System.IO.Path]::GetFullPath(`"..\..\..\tools-local\internet\windows\get-tool.ps1`")) " + "`r`n"
        }
        "MyCustomTool"
        {
...
try
{
    ...
    $toolPath = & ..\..\..\tools-local\Search-Tool.ps1 -SearchLocations $searchLocations -ToolName $ToolName -DeclaredVersion $declaredVersion -StrictToolVersionMatch $StrictToolVersionMatch
    ...
    $toolPath = & ..\..\..\tools-local\Get-Tool.ps1 -DownloadURLs $downloadUrls -ToolName $ToolName -DeclaredVersion $declaredVersion -StrictToolVersionMatch $StrictToolVersionMatch
    if (IsPathNullOrWhiteSpace -ToolPath $toolPath)
    {
        return $toolPath
    }
